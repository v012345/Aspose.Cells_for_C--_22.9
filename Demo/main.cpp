...
//{
//	intrusive_ptr<Aspose::Cells::IWorkbook> wkb = Factory::CreateIWorkbook(sourcePath->StringAppend(new String("input.xls")));
//	wkb->Save(resultPath->StringAppend( new String("Output.xlsx")));
...
int main(int argc, char **argv)
{
    StringPtr xlsPath = new String("..\\");
    StringPtr xlsxPath = new String("xlsx\\");
    for (size_t i = 1; i < argc; i++)
    {
        //cout << argv[i] << endl;
        string input = argv[i];
        input.append(".xls");
        intrusive_ptr<Aspose::Cells::IWorkbook> wkb = Factory::CreateIWorkbook(xlsPath->StringAppend(new String(input.c_str())));
        string output = argv[i];
        output.append(".xlsx");
        wkb->Save(xlsxPath->StringAppend(new String(output.c_str())));
    }
    return 0;
}
